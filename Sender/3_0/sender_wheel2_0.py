...
def get_speed(input_wheel,velocidad,last_vel):
    ...
    #input_wheel: Entrada del volante {-1,1}
    #velocidad: posicion de la palanca {0,-1}{1}
    #last_vel: ultima velocidad registrada
    ...
    return [int(1500+vel_rigth),int(1500+vel_left)]
def send_GUI(input_Palanca): #send the information to a client
    
        try:

            MESSAGE = pickle.dumps(input_Palanca)
            ...
            print("fallo de conexion con el GUI")



def send(speed): #send the information to a client

    data = json.dumps({"motI": speed[0], "motD": speed[1]})
